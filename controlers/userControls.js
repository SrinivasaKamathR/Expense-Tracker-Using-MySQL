...
exports.postUser = async (req, res, next) => {
  ...
  try {
    ...
    if (userExist.length > 0) {
      res
        .status(500)
        .json({ errormsg: "User already exists with this email Id" });
    } else {
      const saltrounds = 10;
      bcrypt.hash(password, saltrounds, async (err, hash) => {
        ...
        res
          .status(201)
          .json({ successMsg: "User created successfully", userDetails });
      });
    }
  } catch (err) {
      ...
      bcrypt.compare(password, registeruserExist[0].password, (err, result) => {
        if (result == true) {
          return res.status(201).json({
            ...
            token: generateAccessToken(registeruserExist[0].id),
          });
        } else {
          return res.status(401).json({
            errMsg: "You entered wrong password. Try again",
            err: err,
          });
        }
      });
